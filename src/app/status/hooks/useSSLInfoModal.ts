...
export const useSSLInfoModal = (
  ...
  const subjectInfo: SSLModalDataProp[] = [
    ...
    { key: 'Subject Alternative Name', value: sslInfo.subject.sans ?? '-' },
  ];
  const issuerInfo: SSLModalDataProp[] = [
    { key: 'Common Name (CN)', value: sslInfo.issuer.common_name ?? '-' },
    { key: 'Organization (0)', value: sslInfo.issuer.org ?? '-' },
  ...
  const validityInfo: SSLModalDataProp[] = [
    ...
    { key: 'Expiry Date', value: formatDate(sslInfo.info.validFrom!) },
