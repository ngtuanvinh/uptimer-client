import { useSSLInfoModal } from "@/app/status/hooks/useSSLInfoModal";
...
type SSLModalDataProp = { key: string; value: string };
    ...
    <div>
      ...
      {body.map((data: SSLModalDataProp, index: number) => (
        <div key={index} className="md:flex md:justify-between text-sm">
          <div className="py-3">{data.key}:</div>
          <div className="py-3">{data.value}</div>
        </div>
      ))}
    </div>
  );
